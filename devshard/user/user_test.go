...
// TestFlushPendingDiff_MovesMsgConfirmStartIntoDiff verifies that
// FlushPendingDiff turns a pending MsgConfirmStart into a real diff so that
// verifiers can see the inference as StatusStarted during execution-timeout
// vote collection.
func TestFlushPendingDiff_MovesMsgConfirmStartIntoDiff(t *testing.T) {
	// Executor (host 1, nonce 1 % 3 = 1) uses a failing engine so that
	// MsgConfirmStart is produced (receipt signed) but MsgFinishInference is
	// never added to the mempool.
	engines := make([]devshard.InferenceEngine, 3)
	engines[0] = stub.NewInferenceEngine()
	engines[1] = stub.NewFailingEngine(fmt.Errorf("ml failure"))
	engines[2] = stub.NewInferenceEngine()
	session, _, _ := setupSessionWithEngine(t, 3, 100000, 100, engines)
	ctx := context.Background()
	...
	// Send inference. Executor signs receipt (confirmedAt > 0) but fails
	// execution, so only MsgConfirmStart ends up in pendingTxs (no finish).
	_, err := session.SendInference(ctx, params)
	require.NoError(t, err)

	// After SendInference, pendingTxs should contain MsgConfirmStart but NOT
	// MsgFinishInference.
	...
	require.True(t, hasConfirm, "pending should have MsgConfirmStart after failed execution")
	require.False(t, hasFinish, "pending should NOT have MsgFinishInference after failed execution")

	// Before flush, session has one diff (nonce 1 with MsgStartInference).
	require.Len(t, session.Diffs(), 1, "should have exactly one diff before flush")

	// Flush: turns MsgConfirmStart into a new diff at nonce 2.
	require.NoError(t, session.FlushPendingDiff())

	// After flush, session has two diffs and no pending txs.
	diffs := session.Diffs()
	require.Len(t, diffs, 2, "FlushPendingDiff should have added a second diff")
	require.Empty(t, session.PendingTxs(), "pendingTxs should be cleared after flush")

	// The new diff (nonce 2) must contain MsgConfirmStart for inference 1.
	diff2 := diffs[1]
	require.Equal(t, uint64(2), diff2.Nonce)
	hasCfm := false
	for _, tx := range diff2.Txs {
	...
	require.True(t, hasCfm, "diff 2 should contain MsgConfirmStart for inference 1")

	// Calling FlushPendingDiff again with empty pending is a no-op (nonce unchanged).
	nonceBefore := session.Nonce()
	require.NoError(t, session.FlushPendingDiff())
	require.Equal(t, nonceBefore, session.Nonce(), "no-op flush must not advance nonce")
	require.Len(t, session.Diffs(), 2, "no-op flush must not add a diff")
}
