		...
		for i := 1; i <= 3; i++ {
			...
			for {
				...
				if finished_report.InferenceId == probe_id {
					...
					//log.Printf("Tx found in blocks %s %d %d %s\n", probe_id, finished_report.StartInferenceBlockHeight, finished_report.FinishInferenceBlockHeight, finished_report.RequestTimestamp)

					// fetch the block header from the last block among start/finish blocks.
